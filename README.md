...
Screenshots/Mockups (Text-based for simplicity):
...
Follow these steps to run the app on your machine. This assumes you have Flutter installed (if not, visit flutter.dev for setup).

Clone or Create the Project:

Open a terminal and run: flutter create todo_app
Navigate into the folder: cd todo_app
Add Dependencies:
...
Replace the contents of lib/main.dart, lib/models/task_model.dart, lib/controllers/task_controller.dart, lib/screens/home_screen.dart, lib/screens/edit_task_screen.dart, and lib/screens/completed_tasks_screen.dart with the code provided in the project files (from our previous messages).
...
On the home screen, type a task (e.g., "Buy groceries") in the text field.
Press the "Add Task" button or hit Enter. The task appears in the list.
Marking Tasks as Done:

Tap the checkbox next to a task. It gets a strikethrough and moves to "completed" status.
Editing Tasks:

Tap on a task's title. It opens the edit screen with the title pre-filled.
Change the text and tap "Save." The app returns to the home screen with the update.
Deleting Tasks:

Swipe left on a task. A red delete background appears—release to confirm.
Viewing Completed Tasks:

Tap the check-circle icon in the app bar (top-right).
See only completed tasks. Tap one to unmark it (removes from completed list).
Persistence:
